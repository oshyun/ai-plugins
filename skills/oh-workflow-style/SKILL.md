...
> | 빌드·테스트 전 simplify 스킬 실행 | simplify 없이 빌드·테스트·머지 진행 금지 |
...
| "simplify는 빌드 후에 해도 되겠다" | 빌드·테스트 전에 반드시 한다. 순서는 고정이다. |
| "simplify는 머지 직전에만 하면 되겠다" | 빌드·테스트 전에도 반드시 한다. 두 시점 모두 고정이다. |
...
- **빌드·테스트 전 순서: simplify → 빌드·테스트. 이 순서를 생략하거나 바꾸는 것은 허용되지 않는다.**
  1. **`simplify` 스킬을 `Skill` 도구로 호출한다.** 스킬이 없는 환경에서는 reuse·simplification·efficiency·altitude 4개 관점으로 diff를 직접 검토해 수정한다.
  2. simplify 완료 후 빌드·테스트를 실행한다.
- **머지 전 순서: rebase → simplify → 빌드·테스트 → 사용자 승인 → merge. 이 순서를 생략하거나 바꾸는 것은 허용되지 않는다.**
  ...
  3. 빌드·테스트를 실행해 simplify 이후에도 동작이 정상임을 확인한다.
  4. 변경 요약(무엇을 바꿨는지)을 사용자에게 보여주고 머지 승인을 **명시적으로** 받는다. 사용자가 응답하기 전까지 머지하지 않는다.
  5. 승인 후 main tree에서 `--no-ff` merge → push → worktree remove 진행.
