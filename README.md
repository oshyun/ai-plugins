...
플러그인 이름이 스킬 네임스페이스가 된다 → 스킬 호출: `/oshyun:dev-style`.
...
```
claude plugin validate ./plugins/oshyun
```

## 확장 — 스킬/커맨드/에이전트 추가

`oshyun` 플러그인은 확장형이다. 새 컴포넌트는 플러그인 루트에 추가한다.

- 스킬: `plugins/oshyun/skills/<이름>/SKILL.md`
- 커맨드: `plugins/oshyun/commands/<이름>.md`
- 에이전트: `plugins/oshyun/agents/<이름>.md`
- 훅: `plugins/oshyun/hooks/hooks.json`
