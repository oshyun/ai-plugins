...
plugins/
  oh-dev-rule/                         ← 확장형 개인 플러그인
    .claude-plugin/plugin.json
    skills/
      dev-style/SKILL.md             ← 개발 습관·코딩 패턴·git 워크플로우
```

플러그인 이름이 스킬 네임스페이스가 된다 → 스킬 호출: `/oh-dev-rule:dev-style`.
...
/plugin install oh-dev-rule@oshyun
```
...
```
claude plugin validate ./plugins/oh-dev-rule
```

## 확장 — 스킬/커맨드/에이전트 추가

`oh-dev-rule` 플러그인은 확장형이다. 새 컴포넌트는 플러그인 루트에 추가한다.

- 스킬: `plugins/oh-dev-rule/skills/<이름>/SKILL.md`
- 커맨드: `plugins/oh-dev-rule/commands/<이름>.md`
- 에이전트: `plugins/oh-dev-rule/agents/<이름>.md`
- 훅: `plugins/oh-dev-rule/hooks/hooks.json`
